...
router.post('/login', (req, res) => {
	...
	extractData()
		...
		.catch((err) => res.send("Login invalid: " + err));
});
...
router.post('/register', (req, res) => {
	...
	const salt = _ => new Promise((resolve, reject) => {
	...
	const dbInsert = ([username, hash, salt]) => new Promise((resolve, reject) => {
		return new Account({
			username: username,
			password: hash,
			salt: salt,
	...
	const respond = (_) => new Promise((resolve, reject) => {
	...
	verifyUnique(username)
		...
		.catch((err) => res.send("Register fail: " + err));
});
